...
```markdown
- 前置模组（需求大时才触发）：需求超出原版/Forge 能力时（GUI 配置界面、配饰槽、游戏内手册、合成查询兼容、多方块机器、自定义矿物世界生成 等），先停下 Read `references/prerequisite-mods.md`，把"原生写 vs 引前置"的利弊摆给用户，默认倾向原版原生，用户拍板。小活儿不命中就跳过。
```
...
读 `references/prerequisite-mods.md`，确认含：取舍说明、映射表（5 个前置）、离线兜底、工程三处改动、第三方核实纪律——四部分齐全且无占位符残留（`<modid>`/`前置group:前置artifact:版本号` 是与 SKILL.md 一致的占位风格，非计划占位符）。
...
追加（指针与 Task 1 一致，直接给文件路径，作为错过 Task 1 触发条后的第二道安全网）：
...
Red Flags 列表下方的收尾句原本只归纳旧 4 条（javap/版本/贴图），新增前置条后需扩写。把这一句：
...
读改动后 Red Flags 列表，确认 5 条齐全，收尾句已含"评估是否该引前置"涵盖前置维度。

- [ ] **Step 5: Commit**

```bash
git add SKILL.md
git commit -m "docs: Red Flags 补'需求超原版却埋头从零写'并扩写收尾句
...
- **占位符**：代码块里的 `<modid>`/`版本号`/`前置group:前置artifact` 是 SKILL.md 现有写法就用的占位风格（参见 SKILL.md 资源结构段 L159/L163/L178 的 `<modid>`/`<name>` 用法），非计划占位符。✓
- **信号词一致性（如实）**：三处对"需求超出原版/Forge"的信号是**子集关系**，非完全一致——Task 1 触发条列 6 个最全（含多方块机器/自定义矿物世界生成）；references 映射表只 5 行（表未覆盖的信号靠"离线兜底/联网自查"规则承接，note 已补多方块机器等例子）；Red Flags 只列 3 个代表性信号。这是有意的：触发条要全（用于识别）、映射表只列有候选前置的、Red Flags 只举代表（防漏触发）。口径方向一致，详略分层。✓
- **离线兜底口径已修**：references 文件已将离线兜底从映射表 blockquote 提升为独立小节，与 spec 的 (d) 部分结构对齐。✓
